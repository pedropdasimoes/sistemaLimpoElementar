<template>
    <div class="table-container">
        ...
        >
            ...
            <div class="table-top-container">
                ...
                <div v-if="searchable.length > 0" class="table-top-left">
                    ...
                    <div
                        v-if="showPagedItemChooser"
                        class="table-top-pagination-ul"
                    >
                        <ul>
                            <li
                                v-for="i in 5"
                                class="table-top-pagination-li"
                                @click="
                                    pagedItem = i * 5;
        ...
        <!--Table-->
        <div :class="['overflow-x-visible', calculatedTableStyle]">
            <table class="w-full min-h-fit">
                <!--Header-->
                <thead>
                    <!--Header Row-->
                    ...
                    <tr
                        ...
                        :class="[
                            ...
                            zebra && index % 2 === 0
                                ? 'bg-opacity-0'
                                : 'bg-opacity-10',
                            paginatedContent.length > index + 1 &&
                                border &&
                                'border-b'
                        ]"
                    >
                        ...
                        >
                            <div
                                :class="[
                                    'flex px-4 py-2',
                                    header[i - 1].align === 'right'
                                        ? 'justify-end'
                                        : header[i - 1].align === 'center'
                                        ? 'justify-center'
                                        : 'justify-start'
                                ]"
                            >
        ...
        <div
            v-if="filteredContent.length > pagedItem"
            class="flex py-4 justify-center"
        >
            <t-paginate
                v-model="page"
                :color="paginationStyle"
                :range="5"
    ...
    computed: {
        ...
        paginatedContent() {
            ...
            return content.slice(
                (this.page - 1) * this.pagedItem,
                this.pagedItem * this.page
            );
        },
        filteredContent() {
            return this.content.filter(item => {
                ...
                for (let i = 0; this.searchable.length > i; i++) {
                    query =
                        item[this.searchable[i]]
                            .toLowerCase()
                            .includes(this.search.toLowerCase()) || query;
                }
