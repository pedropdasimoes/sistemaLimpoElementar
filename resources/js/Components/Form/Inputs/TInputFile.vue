<template>
    <div>
        ...
        <div
            ...
            :class="[
                'flex justify-center items-center my-2 overflow-hidden',
                radiusStyle,
            ]"
        >
            <img
                v-if="files[0].type.split('/')[0] === 'image'"
                :class="['max-h-36 w-auto p-1 border', radiusStyle]"
        ...
        <div :class="['flex border items-center overflow-hidden', radiusStyle]">
            ...
            <div
                v-else
                class="flex px-4 overflow-hidden whitespace-nowrap w-full"
            >
                {{
                    files.length === 0
        ...
        <transition name="fade">
            <div
                ...
                :class="['flex flex-col border w-full p-4 mt-2', radiusStyle]"
            >
                <transition-group name="fade" tag="div">
                    <div
                        v-for="(item, index) in files"
                        :class="[
                            'flex flex-wrap justify-between items-center border p-2 w-full my-1',
                            radiusStyle,
                        ]"
                        :key="item.index"
                    >
                        ...
                        <div class="flex justify-start items-center gap-4">
                            ...
                            <div v-if="preview">
                                <img
                                    v-if="item.type.split('/')[0] === 'image'"
                                    :class="[
                                        'h-14 w-14 p-1 border object-cover',
                                        radiusStyle,
                                    ]"
                                    :src="item | urlGenerator"
                                />
                                <t-video-icon
                                    v-else-if="
                                    ...
                                    "
                                    :class="[
                                        'w-14 h-14 p-3 border text-gray-600',
                                        radiusStyle,
                                    ]"
                                />
                                <t-audio-icon
                                ...
                                <div
                                    v-else
                                    :class="[
                                        'relative flex justify-center items-center w-14 h-14 border text-gray-600',
                                        radiusStyle,
                                    ]"
                                >
    ...
    filters: {
        ...
        upperCase(value) {
            return value.toUpperCase().split(".")[
                value.toUpperCase().split(".").length - 1
            ];
        },
    },
};
