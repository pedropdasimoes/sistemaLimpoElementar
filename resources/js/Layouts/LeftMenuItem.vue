<template>
    <div>
        ...
        <!--
            ...
            para usar permissões nos menus laterais usar esse v-if:

            v-if="
                item.type === 'dropdown' &&
                ($page.props.auth.user.dados.permissao.includes(
                    item.permissao
                ) ||
                    item.permissao == 0)
            "
        ...
        <div
            ...
            @click="
                ...
                showingLeftMenu === 'false' && showingSubMenu === false
                    ? (showingSubMenu = true)
                    : '';
            "
            :class="[
                'left-menu-item',
                {
                    ...
                    'text-white hover:text-gray-700 hover:bg-gray-100 rounded-md':
                        !showingSubMenu,
                },
            ]"
        >
            ...
            <transition name="left-menu">
                ...
                >
                    <div :class="['w-full', item.icon ? 'ml-3' : '']">
                        {{ item.label }}
                    </div>
    ...
    computed: {
        classes() {
            ...
            if (route().current(this.item.link)) {
                style =
                    "bg-white bg-opacity-10 text-white font-semibold border border-opacity-70 border-blue-500";
            } else {
                style =
                    "text-gray-300 bg-opacity-10 hover:bg-gray-200  hover:text-white";
            }
            return style;
        },
    },
